...
from models.q_critic_shyper_gated import SHyperQCriticGated
...
def train_sac_shyper_gated_fixedscale_batch(
    num_iterations=20,
    ...
    print(
        f"GATED SHyper | Fixed scale: jobs={num_jobs}, machines={num_machines}, workers={num_workers}, "
        f"ops/job in [{min_ops_per_job}, {max_ops_per_job}]"
    )

    actor = SHyperActorGated(hidden_dim=hidden_dim)
    q1 = SHyperQCriticGated(hidden_dim=hidden_dim)
    q2 = SHyperQCriticGated(hidden_dim=hidden_dim)
    ...
    for iteration in range(num_iterations):
        ...
        if eval_avg < best_eval_avg:
            ...
            torch.save(agent.actor.state_dict(), "best_fixedscale_gated_sac_actor.pt")
            torch.save(agent.q1.state_dict(), "best_fixedscale_gated_sac_q1.pt")
            torch.save(agent.q2.state_dict(), "best_fixedscale_gated_sac_q2.pt")
...
if __name__ == "__main__":
    train_sac_shyper_gated_fixedscale_batch(
        num_iterations=20,
        batch_instance_size=8,
        episodes_per_instance_batch=5,
        updates_per_episode_round=2,
