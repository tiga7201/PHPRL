...
def main():
    ...
    # collect Step 1 diversity data for several instances
    ...
    print("Replay buffer size before reward learning:", len(replay_buffer))

    reward_model, optimizer, summary = run_reward_learning_step(
        replay_buffer=replay_buffer,
        hidden_dim=64,
        num_layers=2,
        lr=1e-4,
    ...
    # inspect first few relabeled transitions
    first_items = replay_buffer.get_all()[:5]
    print("\nFirst few transitions after relabel:")
    for item in first_items:
        print({
            ...
            "reward_obj": None if item["reward_obj"] is None else round(item["reward_obj"], 4),
        })
