...
def train_sac_shyper_fixedscale_batch(
    num_iterations=30,
    ...
    print(
        f"Fixed scale: jobs={num_jobs}, machines={num_machines}, workers={num_workers}, "
    ...
    # fixed evaluation set under the same scale
    ...
    for iteration in range(num_iterations):
        # refresh the whole training instance batch every few iterations
        ...
        for episode_round in range(episodes_per_instance_batch):
            # solve the same batch of instances once
            ...
            # update after one full sweep over the current training batch
        ...
        if eval_avg < best_eval_avg:
            ...
            torch.save(agent.actor.state_dict(), "best_fixedscale_sac_shyper_actor.pt")
            torch.save(agent.q1.state_dict(), "best_fixedscale_sac_shyper_q1.pt")
            torch.save(agent.q2.state_dict(), "best_fixedscale_sac_shyper_q2.pt")
...
if __name__ == "__main__":
    train_sac_shyper_fixedscale_batch(
        num_iterations=20,
        batch_instance_size=8,
        episodes_per_instance_batch=5,
        updates_per_episode_round=2,
