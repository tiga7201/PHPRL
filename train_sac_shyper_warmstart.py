...
def train_sac_shyper_warmstart(
    num_episodes=300,
    ...
    tau=0.005,
    alpha=0.1,
    bc_actor_path="bc_optimal_actor.pt",
):
    set_seed(42)
    ...
    actor.load_state_dict(torch.load(bc_actor_path, map_location="cpu"))
    ...
    # check initial greedy before RL
    initial_greedy = evaluate_greedy(env, agent, num_episodes=5)
    print(f"Initial greedy after BC warm-start: {initial_greedy:.4f}")
    ...
    for episode in range(num_episodes):
        ...
        if (episode + 1) % 20 == 0:
            ...
            if greedy_makespan < best_greedy:
                ...
                torch.save(agent.actor.state_dict(), "best_sac_shyper_warm_actor.pt")
                torch.save(agent.q1.state_dict(), "best_sac_shyper_warm_q1.pt")
                torch.save(agent.q2.state_dict(), "best_sac_shyper_warm_q2.pt")
