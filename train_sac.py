...
def collect_one_episode(env, agent, buffer, greedy=False):
    state = env.reset()
    done = False
    total_reward = 0.0
    last_info = None
...
def train_sac(
    ...
    for episode in range(num_episodes):
        total_reward, makespan = collect_one_episode(env, agent, buffer, greedy=False)
        ...
        if (episode + 1) % 20 == 0:
            ...
            if greedy_makespan < best_greedy:
                ...
                torch.save(agent.actor.state_dict(), "best_sac_actor.pt")
                torch.save(agent.q1.state_dict(), "best_sac_q1.pt")
                torch.save(agent.q2.state_dict(), "best_sac_q2.pt")
